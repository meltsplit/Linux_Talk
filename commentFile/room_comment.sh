...
sendMessage(){
	...
	bash msgsend.sh #서버로 보내는 msgsend.sh를 실행
}
...
deleteMessage(){
	...
	bash msgsend.sh	#chatLog파일에 변화가 생겼음으로 서버에 chatLog.txt를 보낸다
...
# lastLine에 해당하는 채팅 화면 최하단에 표시된다.
...
showChat(){
	...
	#만약 채팅방이 빈방이 아니라면 접근하지 못하게 하여 에러 가능성을 줄인다. chatCount는 그 방의 메세지 개수를 저장하는 변수이다.
	if [ "" != "$chatCount" ]; then 
...
do  
	...
	if [[ -z ${KEY} ]]; then  # [Enter] 누름
		sendMessage  #함수 이동.
		chatCount=`wc -l < "chatLog_${roomName}.txt"` # chatLog의 파일이 수정되었으니 채팅 개수 초기화
